...
export function getRecoveryActions(
  ...
  switch (error.code) {
    ...
    case "UNSUPPORTED_FORMAT":
      if (context?.onChangeFormat && context?.availableFormats) {
        const fallbackFormat =
          context.availableFormats.find((f) => f === "jpeg") || context.availableFormats[0];
        if (fallbackFormat) {
          actions.push({
    ...
    case "TIMEOUT_ERROR":
      ...
      if (context?.onChangeFormat && context?.availableFormats) {
        const fallbackFormat =
          context.availableFormats.find((f) => f === "png") || context.availableFormats[0];
        if (fallbackFormat) {
          actions.push({
            label: `Try ${fallbackFormat.toUpperCase()}`,
            action: () => context.onChangeFormat!(fallbackFormat),
...
export function handleGenericError(
  ...
  if (error instanceof Error) {
    ...
    if (message.includes("network") || message.includes("fetch")) {
      return new ImageConverterError(
        "NETWORK_ERROR",
        "Network error",
        error.message,
        context,
